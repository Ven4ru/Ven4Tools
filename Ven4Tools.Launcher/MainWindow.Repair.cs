...
namespace Ven4Tools.Launcher
{
    ...
    public partial class MainWindow : IClientRepairExecutor
    {
            ...
            ClientIntegrityReport report, CancellationToken token)
        {
            ...
            // Гейт ставится ДО вопроса «закрыть клиент?» ниже: пока диалог открыт,
            // повторная проверка из окна настроек начала бы хешировать папку, которую
            // эта починка сейчас начнёт переписывать (см. комментарий у поля).
            _integrityOperationRunning = true;
            try
            {
