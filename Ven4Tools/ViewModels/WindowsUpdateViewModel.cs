...
namespace Ven4Tools.ViewModels
{
    ...
    public sealed class WindowsUpdateViewModel : ViewModelBase
    {
        ...
        public async Task InitializeAsync()
        {
            ...
            {
                // Модального вопроса при первом открытии вкладки нет намеренно: режим
                // переключается в «Настройки → 🛡️ Обновления Windows» (SystemTab), тем же
                // способом, что и «Область каталога». Модалка на входе — разовая фрикция
                // в момент, когда пользователь пришёл за другим, и разницу между режимами
                // ему объяснять негде; выпадающий список в настройках доступен всегда и
                // виден рядом с остальными переключателями поведения.
                // Дефолт — консервативный: без явного выбора клиент ничего не качает
                // заранее (курс на отсутствие незапрошенного сетевого трафика).
