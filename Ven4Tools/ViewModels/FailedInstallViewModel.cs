...
namespace Ven4Tools.ViewModels
{
    ...
    public sealed class FailedInstallViewModel : INotifyPropertyChanged
    {
        ...
        private async Task ExecuteRetryAsync()
        {
            ...
            {
                // Не защита от падения приложения — её берёт на себя общий перехват
                // в RelayCommand.FromAsync (см. конструктор). Этот catch нужен ради
                // побочного эффекта: показать причину пользователю в RetryStatus,
                // чего общий обработчик, пишущий только в журнал, не делает.
