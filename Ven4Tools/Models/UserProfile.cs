namespace Ven4Tools.Models
{
    public class UserProfile
    {
        ...
        // Защита от отката каталога (anti-rollback): последняя успешно применённая
        // версия master.json. Клиент отвергает каталог с версией СТРОГО меньше этой,
        // даже если его подпись валидна (старый валидно подписанный каталог мог бы
        // иначе пройти проверку сегодня — downgrade-атака). 0 = памяти ещё нет
        // (первый запуск), в этом случае принимается любая версия.
