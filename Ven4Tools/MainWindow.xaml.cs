...
namespace Ven4Tools
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void Window_Closing_Extended(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            // Предупреждение при закрытии во время активной установки. Не только пачка
            // каталога: общий семафор держат и «Установленные» (обновление/удаление/
            // импорт), и история, и карточка приложения, и пины, и Office, и установка
            // патчей Windows Update — закрытие посреди любой из них обрывало установку
            // молча. Пачку каталога проверяем отдельно: она поднимает IsInstalling ещё
            // до захвата семафора (диалог точки восстановления).
            if (_catalogTab?.IsInstalling == true || InstallationService.IsBusy)
            {
