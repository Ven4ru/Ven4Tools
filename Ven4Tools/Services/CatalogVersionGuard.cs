...
namespace Ven4Tools.Services
{
    ...
    ///
    /// Модель угрозы — ДРУГОЙ пользователь Windows или бэкап/перенос файла на другую
    /// машину, не более привилегированный процесс ТОГО ЖЕ пользователя: DataProtectionScope.
    /// CurrentUser доступен любому коду, работающему от имени того же пользователя
    /// (Entropy — открытая константа в этом файле), и удаление catalog_guard.dat
    /// сбрасывает ReadFromDisk() к 0 (см. IsVersionAcceptable в CatalogLoaderService,
    /// принимающий любую версию при lastSeen <= 0) — то есть тем же процессом, что мог
    /// бы прислать поддельный каталог, счётчик так же можно и сбросить/подделать.
    /// Осознанный компромисс: обычная (недорогая) защита от порчи файла и от переноса
    /// state на чужую машину — не крипто-гарантия против скомпрометированного
    /// same-user-процесса.
    /// </summary>
