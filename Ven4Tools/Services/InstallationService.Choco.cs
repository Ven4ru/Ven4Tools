    ...
    public partial class InstallationService
    {
            ...
            CancellationToken token)
        {
            ...
            if (chocoOk)
            {
                ...
                // ID, исполняемый файл не найден, таймаут, общее исключение) —
                // RunChocoInstallAsync уже залогировал точную причину выше по стеку.
                // Причины разные, единого честного текста для них нет, поэтому
                // расшифровывать нечего — как и в InstallFromWingetAsync (см. там же).
                // В ChocoErrorMapper записи для -1 сознательно нет (см. комментарий там).
