...
namespace Ven4Tools.Services
{
    public partial class InstallationService : IDisposable
    {
        ...
        // и каталогом/историей, и УСТАНОВКОЙ патчей Windows Update, т.к. оба используют
        // общую MSI-подсистему и не должны ставить/удалять параллельно.
        // Фоновое СКАЧИВАНИЕ патчей сюда намеренно не входит: оно msiexec не трогает и
        // живёт на своём семафоре (см. WindowsUpdateService) — иначе многоминутная
        // загрузка блокировала бы установку приложений без причины и с ложным
        // сообщением «дождитесь завершения текущей установки».
