...
namespace Ven4Tools.Services
{
    public partial class InstallationService
    {
            ...
            string outcomeCheckId, InstalledBaseline baseline, CancellationToken token)
        {
            ...
            //
            // В журнал неудач (failed_installs.json) это НЕ пишется по той же
            // причине: пустой sha256 — осознанное состояние каталога для записей с
            // «вечной» ссылкой без версии в URL (firefox-latest, chrome/install/latest
            // и т.п.), где зафиксированный хеш протухает при каждой сборке вендора.
            // Установка при этом штатно проходит через winget/choco, и запись
            // «В каталоге не указан SHA256» оседала бы в журнале навсегда после
            // КАЖДОЙ успешной установки таких приложений — а лаунчер предлагает
            // опубликовать этот журнал публичным issue на GitHub. Реально
            // провалившаяся цепочка по-прежнему журналируется один раз в конце,
            // источником "all-sources" (см. InstallationService.cs).
