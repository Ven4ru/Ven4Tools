...
namespace Ven4Tools.Views.Tabs
{
    public partial class WindowsUpdateTab : UserControl
    {
        ...
        private async void WindowsUpdateTab_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            // режим обещает блокировать. Ручная кнопка «Проверить» ниже НЕ гейтится —
            // это осознанное решение: пользователь сам инициирует запрос, и вкладка
            // «Windows Update» в тексте параноидального режима не заявлена как
            // полностью заблокированная (см. SystemTab.xaml, список исключений).
            if (ProfileService.Current.ParanoidMode)
            {
                txtStatus.Text = "Автопроверка отключена (параноидальный режим). Нажмите «Проверить обновления» вручную.";
