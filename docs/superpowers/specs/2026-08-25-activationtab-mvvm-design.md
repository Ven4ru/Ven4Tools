...
Второе осознанное отклонение: блокировка кнопки «Проверить статус» на время проверки перестаёт быть синхронной — было `btnCheckStatus.IsEnabled = false` прямо в обработчике клика, стало `IsCheckingStatus = true` → `RaiseCanExecuteChanged()` → `CommandManager.InvalidateRequerySuggested()`, который переспрашивает `CanExecute` с приоритетом `DispatcherPriority.Background`, из-за чего остаётся короткое окно на повторный клик; это принятая цена за устоявшийся в проекте паттерн (тот же, что у `HistoryViewModel`/`IsReinstalling`), а повторный запуск проверки статуса идемпотентен и безопасен.
