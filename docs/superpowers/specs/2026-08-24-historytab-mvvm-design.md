...
## Побочная уборка (не новая правка, следствие переноса)

`BtnReinstall_Click` сейчас сам ловит `OperationCanceledException`/`Exception` — точечная копия try/catch, которую doc-комментарий `RelayCommand.FromAsync` прямо называет ещё не закрытой. При переносе на `RelayCommand.FromAsync` этот дублирующий try/catch убирается — перехват берёт на себя `FromAsync`.
