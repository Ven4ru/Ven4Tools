...
4. **Анимации, требующие живой `UIElement`** (`MotionService.CrossFade`/`MotionService.Pulse`) остаются заботой code-behind, не VM — тот же принцип, что уже применялся к `Loaded`-специфике. VM поднимает два простых события `event Action? ThemeApplied;` и `event Action? ConnectivityStatusUpdated;` (без параметров — не несут данных, чисто нотификация "произошло"), code-behind подписывается и на своих именованных элементах (`Window.GetWindow(this) ?? this` для темы, `pnlConnStatus` для статуса связи) вызывает `MotionService.CrossFade`/`MotionService.Pulse` с теми же параметрами, что в оригинале (220мс / scale 1.015, 160мс).
